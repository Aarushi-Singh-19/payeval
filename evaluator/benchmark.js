  ...
  (result) =>
    result.actual.decision === "REQUIRE_APPROVAL"
).length;

const approvalExpected = results.filter(
  (result) =>
    result.expected.decision === "REQUIRE_APPROVAL"
).length;
...
const approvalHandlingRate =
  approvalExpected === 0
    ...
    : Number(
        (
          (approvalRequired / approvalExpected) *
          100
        ).toFixed(2)
      );
